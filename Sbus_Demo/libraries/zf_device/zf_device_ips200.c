...
*                   BL                  查看 zf_device_ips200.h 中 IPS200_BLk_PIN_SPI  宏定义
*                   电源引脚
*                   VCC                 3.3V电源
*                   GND                 电源地
*                   最大分辨率 320 * 240
...
static void ips200_write_command (const uint8 command)
{
    ...
    else
    {
        ...
        IPS200_RD(1);
        IPS200_WR(0);
        *(volatile uint16 *)IPS200_DATA_ADD = ((uint16_t)command);
        IPS200_WR(1);
        IPS200_CS(1);
        IPS200_RS(1);
    }
...
static void ips200_write_8bit_data (const uint8 dat)
{
    ...
    else
    {
        ...
        IPS200_RD(1);
        IPS200_WR(0);
        *(volatile uint16 *)IPS200_DATA_ADD = ((uint16_t)dat);
        IPS200_WR(1);
        IPS200_CS(1);
    }
}
...
static void ips200_write_16bit_data (const uint16 dat)
{
    ...
    else
    {
        ...
        IPS200_RD(1);
        IPS200_WR(0);
        *(volatile uint16 *)IPS200_DATA_ADD = ((uint16_t)dat>>8);
        IPS200_WR(1);
        IPS200_WR(0);
        *(volatile uint16 *)IPS200_DATA_ADD = ((uint16_t)dat);
        IPS200_WR(1);
...
// @return      void
// Sample usage:                ips200_clear(RGB565_BLACK);
//-------------------------------------------------------------------------------------------------------------------
...
void ips200_init (ips200_type_enum type_select)
{
    ...
    else
    {
        ...
        gpio_init(IPS200_RS_PIN_PARALLEL8, GPO, GPIO_LOW, GPO_PUSH_PULL);       // RS
        ...
        gpio_init(IPS200_CS_PIN_PARALLEL8, GPO, GPIO_HIGH, GPO_PUSH_PULL);      // LCD_CS
