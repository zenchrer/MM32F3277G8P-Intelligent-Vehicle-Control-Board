...
static  uint16 iic_target_addr[2];
...
// @param       data            要写入的数据
// @return      void            
// Sample usage:                iic_write_8bit(IIC_1, 0x01);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_8bit (iic_index_enum iic_n, const uint8 data)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_write_8bit_arrray(IIC_1, data, 64);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_8bit_arrray (iic_index_enum iic_n, const uint8 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       data            要写入的数据
// @return      void            
// Sample usage:                iic_write_16bit(IIC_1,0x0101);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_16bit (iic_index_enum iic_n, const uint16 data)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_write_16bit_arrray(IIC_1, data, 6);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_16bit_arrray (iic_index_enum iic_n, const uint16 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       data            要写入的数据
// @return      void            
// Sample usage:                iic_write_8bit_register(IIC_1, 0x01, 0x01);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_8bit_register (iic_index_enum iic_n, const uint8 register_name, const uint8 data)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_write_8bit_registers(IIC_1, 0x01, data, 6);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_8bit_registers (iic_index_enum iic_n, const uint8 register_name, const uint8 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       data            要写入的数据
// @return      void            
// Sample usage:                iic_write_16bit_register(IIC_1, 0x0101, 0x0101);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_16bit_register (iic_index_enum iic_n, const uint16 register_name, const uint16 data)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_write_16bit_registers(IIC_1, 0x0101, data, 6);
//-------------------------------------------------------------------------------------------------------------------
void iic_write_16bit_registers (iic_index_enum iic_n, const uint16 register_name, const uint16 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// Sample usage:                iic_read_8bit(IIC_1);
//-------------------------------------------------------------------------------------------------------------------
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_read_8bit_array(IIC_1, data, 64);
//-------------------------------------------------------------------------------------------------------------------
void iic_read_8bit_array (iic_index_enum iic_n, uint8 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// Sample usage:                iic_read_16bit(IIC_1);
//-------------------------------------------------------------------------------------------------------------------
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_read_16bit_array(IIC_1, data, 8);
//-------------------------------------------------------------------------------------------------------------------
void iic_read_16bit_array (iic_index_enum iic_n, uint16 *data, uint32 len)
{
    iic_index[iic_n]->ENR |= 0x00000001;                                        // IIC 使能
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       register_name   传感器的寄存器地址
// @return      uint8           返回读取的 8bit 数据
// Sample usage:                iic_read_8bit_register(IIC_1, 0x01);
//-------------------------------------------------------------------------------------------------------------------
uint8 iic_read_8bit_register (iic_index_enum iic_n, const uint8 register_name)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_read_8bit_registers(IIC_1, 0x01, data, 8);
//-------------------------------------------------------------------------------------------------------------------
void iic_read_8bit_registers (iic_index_enum iic_n, const uint8 register_name, uint8 *data, uint32 len)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       register_name   传感器的寄存器地址
// @return      uint16          返回读取的 16bit 数据
// Sample usage:                iic_read_16bit_register(IIC_1, 0x0101);
//-------------------------------------------------------------------------------------------------------------------
uint16 iic_read_16bit_register (iic_index_enum iic_n, const uint16 register_name)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       len             缓冲区长度
// @return      void            
// Sample usage:                iic_read_16bit_registers(IIC_1, 0x0101, data, 8);
//-------------------------------------------------------------------------------------------------------------------
void iic_read_16bit_registers (iic_index_enum iic_n, const uint16 register_name, uint16 *data, uint32 len)
{
    iic_index[iic_n]->ENR |= 0x00000001;                                        // IIC 使能
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       data            要写入的数据
// @return      void            
// Sample usage:                iic_sccb_write_register(IIC_1, 0x01, 0x01);
//-------------------------------------------------------------------------------------------------------------------
void iic_sccb_write_register (iic_index_enum iic_n, const uint8 register_name, uint8 data)
{
    iic_index[iic_n]->ENR &= 0xFFFFFFFE;                                        // IIC 关闭
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];
    iic_delay();
...
// @param       register_name   传感器的寄存器地址
// @return      uint8           返回读取的 8bit 数据
// Sample usage:                iic_sccb_read_register(IIC_1, 0x01);
//-------------------------------------------------------------------------------------------------------------------
uint8 iic_sccb_read_register (iic_index_enum iic_n, const uint8 register_name)
{
    uint8 data = 0;
...
void iic_init(iic_index_enum iic_n, uint8 addr, uint32 speed, iic_scl_pin_enum scl_pin, iic_sda_pin_enum sda_pin)
{
    ...
    iic_target_addr[iic_n] = (iic_index[iic_n]->TAR & 0xFFFFFF00) | (uint32)addr;
    iic_index[iic_n]->TAR = iic_target_addr[iic_n];                             // 写入地址
