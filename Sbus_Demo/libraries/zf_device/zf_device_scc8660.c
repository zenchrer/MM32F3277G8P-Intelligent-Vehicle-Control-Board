...
static volatile uint8     uart_receive_flag;
...
static uint8 scc8660_set_config (int16 buff[SCC8660_CONFIG_FINISH][2])
{
    ...
    if((0xff != receive[1]) || (0xff != receive[2]))
        return 1;
...
// @param       void
// @return      void
// Sample usage:                scc8660_uart_callback();
//-------------------------------------------------------------------------------------------------------------------
void scc8660_uart_callback (void)
{
...
//    }
    ...
    camera_init(SCC8660_IMAGE_SIZE);
    return 0;
}
